...
N = 12;
x = 1:N;
y = randi(2000,1,N)+2000;

% x = [316800,317700,318600,319500,320400,...
%       321300,322200,323100,324000,324900,...
%       325800,326700];
y = [22620.648371, 22838.312565,22810.742194,...
     22516.06855,21940.103572,21076.99459,...
     19929.567654,18509.346124,16836.243679,...
     14937.943406,12848.98659,10609.605879];
y=y.*1e03;


% Uniform breaks
breaks = linspace(1,N,40); % 41 breaks, 40 pieces
% pp1 = splinefit(x,y,breaks);

% Breaks interpolated from data
% pp2 = splinefit(x,y,20);  % 11 breaks, 10 pieces
...
% y1 = ppval(pp1,xx);
% y2 = ppval(pp2,xx);
% plot(x,y.*1e-04,'.',xx,[y1.*1e-04;y2.*1e-04]); hold on;
% % plot(xx,y1,'-r',xx,y2,'-g');
% axis([0,12,min(y.*1e-04)-0.5,max(y.*1e-04)+0.5]), grid on
...
pp6 = splinefit(x,y,8,6);  % Etc.
pp7 = splinefit(x,y,8,7);  % Etc.
pp8 = splinefit(x,y,8,8);  % Etc.
pp  = pchip(x,y);
pz  = spline(x,y);
...
figure(3)
plot(x,y,'.',xx,y3,'-g'), grid on, hold on
plot(x,y,'.',xx,tt,'-.b')
plot(x,y,'.',xx,tz,'-.k')
