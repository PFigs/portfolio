...
class Scripter( Thread ):
	numero_de_clientes_a_escrever=0
	"""
	Obtains the contents of the init file.
	Dispatches every command for the time set in its timestamp.
	"""
	def __init__(self, bank, verbose, benchmark=False, filename="benchmark/benchmark-", mode = 'a'):
		global numero_de_clientes_a_escrever
		...
		self.__runtime = time()
		self.__started = time()
		self.__myidis = Scripter.numero_de_clientes_a_escrever
		Scripter.numero_de_clientes_a_escrever = Scripter.numero_de_clientes_a_escrever+1
		if benchmark:
			self.__filename = filename + str(getpid())
			self.__mode = mode
			self.__Measures = []
		else:
