...
% xiddata = NaN(userparams('MAXSAT'),1);
% yiddata = NaN(userparams('MAXSAT'),1);
for k=1:D.NumLines                 % plot the normalized data
    ...
    if isempty(num)
        ...
        D.HLines(k)=plot(HAxes,xdata,ydata,...
              ...
              'Marker',D.Marker{k});  
%         xiddata(num) = xdata(num);
%         yiddata(num) = ydata(num);
    end
end
% 
% for m = 1:userparams('MAXSAT');
%     if isnan(xiddata(m))
%         continue;
%     end 
%     
%     line('XData',xiddata(m), 'YData',yiddata(m), ...
%         'LineStyle','none', 'Marker','o', 'MarkerSize',16, ...
%         'MarkerFaceColor',rgb('Light Gray'), 'MarkerEdgeColor',rgb('Light Gray'));
%     text(xiddata(m),yiddata(m),['\color{black}',sprintf('%d',m)],...
%         'FontSize',12, 'FontWeight','bold','HorizontalAlign','center', 'VerticalAlign','middle');
% end
...
for kk=1:length(Sfn)
   ...
   case 'RLimit'                                                   % RLimit
      storelast = NaN(userparams('MAXSAT'),2);
...
%          h = waitbar(0,'MMPOLAR: Please wait...');
%          steps = length(D.RData);
         for k=1:length(D.RData)
            D.RDataN(k)={(D.RData{k}-D.RMin)/D.RLimitDiff};
            notvalid = isnan(D.RDataN{k});
            if all(notvalid)
                continue;
            end;
            ...
            storelast(~notvalid,1)=xdata(~notvalid);
            storelast(~notvalid,2)=ydata(~notvalid);
%             waitbar(k / steps);
         end
         
        for m = 1:userparams('MAXSAT')
            if isnan(storelast(m,1)), continue; end;
            line('XData',storelast(m,1), 'YData',storelast(m,2), ...
            'LineStyle','none', 'Marker','o', 'MarkerSize',16, ...
            'MarkerFaceColor',rgb('Light Gray'), 'MarkerEdgeColor',rgb('Light Gray'));
            text(storelast(m,1),storelast(m,2),['\color{black}',sprintf('%d',m)],...
            'FontSize',12, 'FontWeight','bold','HorizontalAlign','center', 'VerticalAlign','middle');
        end 
