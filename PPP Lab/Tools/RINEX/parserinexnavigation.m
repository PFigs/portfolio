    ...
    if version == 2.1
        for k=idx+1:fSize
            ...
            if strcmpi(state,'orbitstart')
                ...
                WD  = weekday(datenum(year,month,day));
                TOC = weekdaytotow(WD,hour,minute,second);
            ...
            elseif strcmpi(state,'orbit7')
                ...
                if time - lasttime > 50*60
                    lasttime = time;
                    iEpoch   = iEpoch + 1;
                    data.eph.time = [data.eph.time (time)];
                    data.eph.data = [data.eph.data {temp}];
                else
                    data.eph.data{iEpoch}(:,satid) = temp(:,satid);
                end
